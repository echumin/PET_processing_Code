function [] = view_sagittal_slice (img, slicenum, varargin)

% File: view_sagittal_slice.m
%
% Description:
%   Display an sagittal slice (or what I think is an sagittal slice) of an 
%   image volume.
%
% Usage:
%   [ ] = view_sagittal_slice (IMG, SLICENUM)
%   [ ] = view_sagittal_slice (IMG, SLICENUM, CMAP)
%
% Inputs:
...
dim = size (img);
img = reshape (img(slicenum,:,:), dim(2), dim(3));
img = img(end:-1:1,end:-1:1)';
imagesc (img);
%imagesc (reshape (img(slicenum,:,:), dim(2), dim(3))(:,end:-1:1)');
axis image;
colorbar;
